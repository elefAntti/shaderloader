...
float dist_model( vec3 pos )
{
    ...
    float fMeta = dist_metab(
        metaball( pos + vec3( sin(time) * 1.0, 1.0, cos(time) * 5.0 ), 1.1 )
        + metadiamond( 
            rotate_y(pos + vec3(
                            sin(time * 2.0) * 1.0,
                            cos(time * 2.0) * 4.0,
                    ...
                    time * 3.0 )
                , 1.0 )
        + metaball( pos + vec3( sin(time * 1.2) * 3.0, sin(time * 1.2) * 1.0, cos(time * 1.2) * -1.0 ), 0.9 )
        + metabox( pos + vec3( sin(time * 1.5) * -1.0, cos(time * 1.5) * -1.5, sin(time * 1.5) * -0.5 ), 1.2 )
        ) / 1.0;
